...
The document scoring mechanism allows Lucene to return highly relevant search results, given a user query, even in large and complex index. In the next post, [Running Apache Lucene at Scale](/tech-blog/apache-lucene-at-scale/), I will look at how Lucene's design makes it suitable to run at scale.
