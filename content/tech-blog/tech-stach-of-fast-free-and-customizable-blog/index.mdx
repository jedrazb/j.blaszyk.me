...
Check this post's [source](https://github.com/jedrazb/personal-blog/blob/master/content/tech-blog/tech-stach-of-fast-free-and-customizable-blog/index.mdx?plain=1#L44) to verif that it's just a React `<SliderPicker />` defined in MDX file.
...
CloudFlare is an awesome reverse cache proxy and CDN that provides DNS and HTTPS. Once you add your static site make sure to enable Cloudflare to cache it - and effectively serve it from CDN - by adding a page rule to “Cache Everything” - this should visibly speed up the load times. Whenever you make a change to your site you would need to remember to manually purge the cache in Cloudflare (it probably can be automated using Github Actions).
...
- The blog is `static` - it means it's fast but also it means there is no server able to serve dynamic content. User accounts, reactions, comments - all of that can't and won't be natively supported by Gatsby. Though there are many (freemium and paid) solutions on the market which at least allow you to embed reactions and comments.
