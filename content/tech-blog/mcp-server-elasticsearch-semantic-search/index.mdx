...
I prepared a [simple demo repo](https://github.com/jedrazb/elastic-semantic-search-mcp-server) that's a good starting point to crawl an example dataset, embed the crawled content with Elastic's `semantic_text`, and expose the tool to semantically search the data via an MCP server.
