...
date: '2022-01-16'
spoiler: Exploring one of the core concepts in Lucene - inverted index, insertions and deletions.
cta: 'apache lucene'
...
Below there is a visualisation of how Lucene handles insertions, deletion and segment merges (taken from this great [blog post](https://blog.mikemccandless.com/2011/02/visualizing-lucenes-segment-merges.html)). Each segment is a bar, whose height is the size (in MB) of the segment. Segments on the left are the largest; as new segments are flushed, they appear on the right. The dark grey band on top of each segment shows the proportion of deletions in that segment.

![Lucene segment merges](./segment_merges.gif)

### Lucenes' data storage architecture: pros and cons
...
- (+) DocValues optimization can help with efficient sorting and faceting. Sorted fields can help with retreving whole documents.
