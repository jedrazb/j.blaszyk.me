...
In order to configure the index we need to run several requests in order. We need to run commands in order, for the primary and each of the replicas:
...
Great! Now we have a running nrtsearch cluster and our index is running and replicated between primary and replicas. Let's move on to data ingestion.
