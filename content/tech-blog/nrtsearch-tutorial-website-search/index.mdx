...
Start the UI by running `make grpcox`. It will be accessible on `localhost:6969`. For our tutorial nrtsearch nodes running in `docker` the hostname will be always `host.docker.internal` and the port will be `8000` for the primary and `8001-2` for the two replicas.

Let's connect to one of the replicas and fire a simple search request to explore the index.

- Connect to replica-1 at `host.docker.internal:8002`
