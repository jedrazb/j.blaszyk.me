...
Great! Now we have a local nrtsearch cluster with nodes dedicated to data ingestion and search. Now, let's design the index schema and start the index.
...
Let's use gRPC to communicate with nrtsearch. Since it's not a REST API we can't just `curl` the endpoints. We need to generate the native client code, `python` in this tutorial, to interact with nrtsearch.
...
Before ingesting the data into NRTsearch we need to first create an index and register the fields. The appropriate index schema definition, e.g. using tokenizers and analyzers where needed, is critical for Lucene search performance.
...
- The nested field `url.id` has `_ID` - it specifies a unique resource id. Note, the `url` field has TEXT type as we still want to keyword search within the url
- Each field is searchable, `search: true`, tokenized `tokenize: true` and is stored in column-oriented way `storeDocValues: true`.
- The `content.analyzed` field contains the index-time analyzed webpage content. We define a custom analyzer which removes any html characters (`htmlstrip` charFilter), uses `standard` tokenizer and `kStem`, `stop` and `lowercase` Lucene token filters. In order to support the [Fast Vector Highlighter](https://nrtsearch.readthedocs.io/en/latest/highlighting.html), we set `termVectors`, `store` and `tokenize` properties.

The index schema is defined in [/nrtsearch_index/index_schema.yaml](https://github.com/jedrazb/nrtsearch-tutorial-website-search/blob/master/nrtsearch_index/index_schema.yaml) and looks like this:
