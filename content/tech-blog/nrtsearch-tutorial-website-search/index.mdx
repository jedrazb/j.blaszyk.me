...
In the [Makefile](https://github.com/jedrazb/nrtsearch-tutorial-website-search/blob/master/Makefile) there are commands `nrtsearch_protos` and `nrtsearch_protos` which will execute above steps above and put the generated code into [/nrtsearch_client/nrtsearch_py_grpc](https://github.com/jedrazb/nrtsearch-tutorial-website-search/tree/master/nrtsearch_client/nrtsearch_py_grpc).
...
- The nested field `url.id` has `_ID` - it specifies unique resource id. Note, the `url` field has TEXT type as we still want to do keyword search within the url
