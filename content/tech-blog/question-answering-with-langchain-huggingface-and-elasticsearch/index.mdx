---
title: 'Question Answering System with LangChain, HuggingFace, and Elasticsearch'
date: '2024-05-22'
...
Let's build a retrieval-augmented generation (RAG) system for question answering that can run locally in a notebook. We'll use the content of this blog as our toy knowledge source. By leveraging `langchain` and HuggingFace `transformer` packages, I aim to keep the code minimal and utilize their neat abstractions.

For our vector store, I'm using Elasticsearch Serverless (currently in open preview for free), which will enable semantic search on data embedded with `HuggingFaceEmbeddings`. To maintain the entire pipeline locally in the notebook, we'll use small transformer models for both question answering and a proper LLM (running locally on the M2 Pro Macbook!) for text generation.

You can run all of this for free in your notebook, which is available at the end of this post.
...
- **Retriever**: A vector search engine that retrieves relevant documents from the knowledge source (e.g. a vector store) and passes them as context to the generator.
...
Language models have token limits, and chunking breaks down large documents into smaller, manageable pieces that fit within these limits. Additionally, chunking enhances semantic search by generating embeddings for each chunk, ensuring contextually relevant and accurate retrieval and processing of information.

When selecting `chunk_size`, choose a size within the model’s token limit. Consider the typical length of meaningful text segments: larger chunks capture more context but may lead to inefficiency or truncation, while smaller chunks are better for technical or dense content. For `chunk_overlap`, select a value that preserves context across chunk boundaries.
...
# limit the number of context docs to retrieve to top 1 context doc for paster processing by LLM
retriever = ElasticsearchRetriever.from_es_params(
    index_name=ES_INDEX_NAME,
