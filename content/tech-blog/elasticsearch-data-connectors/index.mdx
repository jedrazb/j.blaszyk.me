...
In this post, I want to focus on one of them: the **data connectors framework**, as it is a simple yet powerful solution to ingestion your data into the Elasticsearch index.

Each of the ingestion methods can be used with in private Elastic deployment or in the Elastic cloud.

## Data connectors framework

[elastic/connectors-python](https://github.com/elastic/connectors-python)

The data connectors framework enables developers to build Elastic-supported connector clients that sync third-party data sources to Elasticsearch. The framework implements common functionalities, such as sync scheduling, integration with Kibana and creation of index mappings, out of the box, so developers can focus on the logic specific to integrating their chosen data source.

(insert screenshot with connector syncing data)
...
The framework **connects** your third-party data source with an Elasticsearch index and keeps it in sync, so that you can focus on search and analytics.
...
- **Control over a deployment** - This service can be easily self-hosted or run in the Elastic cloud. It only needs to be able to discover your Elasticsearch instance over the network.
...
All you need to do is define a custom `DataSource` class in async python and the framework will take care of making it compatible with common functionalities, such as sync scheduling, integration with Kibana and creation of index mappings.
...
To learn more how to write a custom connector, [check implementation of existing connectors](https://github.com/elastic/connectors-python/tree/main/connectors/sources) or refer to [Custom data source connector framework for Elasticsearch](fsd) blog post.

To learn how to contribute new connectors to the framework, refer to the [How to contribute connectors guide](https://github.com/elastic/connectors-python/blob/main/docs/CONTRIBUTING.md) in the `connectors-python` repository.
